...
describe('computePareto', () => {
  test('1 — A(60, 1.0), B(50, 2.0): B worse on both', () => {
    const a = makeRow({ cursorName: 'A', intelligence: 60, aaCostPerTask: 1.0 })
    const b = makeRow({ cursorName: 'B', intelligence: 50, aaCostPerTask: 2.0 })
  ...
  test('2 — A(60, 2.0), B(50, 1.0): neither dominates', () => {
    const a = makeRow({ cursorName: 'A', intelligence: 60, aaCostPerTask: 2.0 })
    const b = makeRow({ cursorName: 'B', intelligence: 50, aaCostPerTask: 1.0 })
    const result = computePareto([a, b], 'intelligence')
  ...
  test('3 — A(60, 1.0), B(60, 2.0): equal score, higher cost dominated', () => {
    const a = makeRow({ cursorName: 'A', intelligence: 60, aaCostPerTask: 1.0 })
    const b = makeRow({ cursorName: 'B', intelligence: 60, aaCostPerTask: 2.0 })
    const result = computePareto([a, b], 'intelligence')
    expect(rowNames(result.frontier)).toEqual(['A'])
    expect(rowNames(result.dominated)).toEqual(['B'])
    expect(rowNames(result.excluded)).toEqual([])
  ...
  test('5 — A(60, 1.0), B(60, 1.0): identical points, both on frontier', () => {
    const a = makeRow({ cursorName: 'A', intelligence: 60, aaCostPerTask: 1.0 })
    const b = makeRow({ cursorName: 'B', intelligence: 60, aaCostPerTask: 1.0 })
    const result = computePareto([a, b], 'intelligence')
  ...
  test('6 — A(null, 1.0), B(50, 2.0): null score excluded, not treated as 0', () => {
    const a = makeRow({ cursorName: 'A', intelligence: null, aaCostPerTask: 1.0 })
    const b = makeRow({ cursorName: 'B', intelligence: 50, aaCostPerTask: 2.0 })
    const result = computePareto([a, b], 'intelligence')
  ...
  test('7 — A(60, null), B(50, 2.0): null cost excluded', () => {
    const a = makeRow({ cursorName: 'A', intelligence: 60, aaCostPerTask: null })
    const b = makeRow({ cursorName: 'B', intelligence: 50, aaCostPerTask: 2.0 })
    const result = computePareto([a, b], 'intelligence')
    expect(rowNames(result.frontier)).toEqual(['B'])
    expect(rowNames(result.dominated)).toEqual([])
  ...
  test('8 — all rows have null cost: frontier empty, all excluded', () => {
    const a = makeRow({ cursorName: 'A', intelligence: 60, aaCostPerTask: null })
    const b = makeRow({ cursorName: 'B', intelligence: 50, aaCostPerTask: null })
    const result = computePareto([a, b], 'intelligence')
  ...
  test('11 — A(70,3), B(60,2), C(50,1), D(55,4)', () => {
    const a = makeRow({ cursorName: 'A', intelligence: 70, aaCostPerTask: 3 })
    const b = makeRow({ cursorName: 'B', intelligence: 60, aaCostPerTask: 2 })
    const c = makeRow({ cursorName: 'C', intelligence: 50, aaCostPerTask: 1 })
    const d = makeRow({ cursorName: 'D', intelligence: 55, aaCostPerTask: 4 })
  ...
  test('12 — frontier output ordering: ascending by aaCostPerTask', () => {
    const a = makeRow({ cursorName: 'A', intelligence: 70, aaCostPerTask: 3 })
    const b = makeRow({ cursorName: 'B', intelligence: 60, aaCostPerTask: 2 })
    const c = makeRow({ cursorName: 'C', intelligence: 50, aaCostPerTask: 1 })
    const result = computePareto([a, b, c], 'intelligence')
    const costs = result.frontier.map((r) => r.aaCostPerTask)
...
describe('computePareto properties', () => {
    ...
    for (let i = 0; i < 20; i++) {
      rows.push(
        makeRow({
          ...
          aaCostPerTask: ((i * 3 + 5) % 50) / 10 + 0.5,
        }),
      )
    }
    const result = computePareto(rows, 'intelligence')
    const eligible = rows.filter((r) => r.intelligence !== null && r.aaCostPerTask !== null)
  ...
  test('14 — as cost increases along frontier, metric strictly increases', () => {
    const rows = [
      makeRow({ cursorName: 'A', intelligence: 70, aaCostPerTask: 3 }),
      makeRow({ cursorName: 'B', intelligence: 60, aaCostPerTask: 2 }),
      makeRow({ cursorName: 'C', intelligence: 50, aaCostPerTask: 1 }),
      makeRow({ cursorName: 'D', intelligence: 55, aaCostPerTask: 4 }),
    ]
    const result = computePareto(rows, 'intelligence')
    for (let i = 1; i < result.frontier.length; i++) {
      const prev = result.frontier[i - 1]!
      const curr = result.frontier[i]!
      expect(curr.aaCostPerTask!).toBeGreaterThan(prev.aaCostPerTask!)
  ...
  test('15 — partition invariant on every case above', () => {
    const cases: ModelRow[][] = [
      [
        makeRow({ cursorName: 'A', intelligence: 60, aaCostPerTask: 1.0 }),
        makeRow({ cursorName: 'B', intelligence: 50, aaCostPerTask: 2.0 }),
      ],
      [
        makeRow({ cursorName: 'A', intelligence: 60, aaCostPerTask: 2.0 }),
        makeRow({ cursorName: 'B', intelligence: 50, aaCostPerTask: 1.0 }),
      ],
      [
        makeRow({ cursorName: 'A', intelligence: 60, aaCostPerTask: 1.0 }),
        makeRow({ cursorName: 'B', intelligence: 60, aaCostPerTask: 2.0 }),
      ],
      [
        makeRow({ cursorName: 'A', intelligence: 60, aaCostPerTask: 1.0 }),
        makeRow({ cursorName: 'B', intelligence: 50, aaCostPerTask: 1.0 }),
      ...
      [
        makeRow({ cursorName: 'A', intelligence: 60, aaCostPerTask: 1.0 }),
        makeRow({ cursorName: 'B', intelligence: 60, aaCostPerTask: 1.0 }),
      ],
      [
        makeRow({ cursorName: 'A', intelligence: null, aaCostPerTask: 1.0 }),
        makeRow({ cursorName: 'B', intelligence: 50, aaCostPerTask: 2.0 }),
      ],
      [
        makeRow({ cursorName: 'A', intelligence: 60, aaCostPerTask: null }),
        makeRow({ cursorName: 'B', intelligence: 50, aaCostPerTask: 2.0 }),
      ],
      [
        makeRow({ cursorName: 'A', intelligence: 60, aaCostPerTask: null }),
        makeRow({ cursorName: 'B', intelligence: 50, aaCostPerTask: null }),
      ],
      [],
      [makeRow({ cursorName: 'A', intelligence: 60, aaCostPerTask: 1.0 })],
      [
